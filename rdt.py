from socket import *
from aux_functions import sock_send, sock_receive
from constants import BUFFER_SIZE,  UDP_IP, UDP_PORT


class RDT:
    # projeto ja se inicia com valores pré-setados
    def __init__(self, isServer=0, addressPort=(UDP_IP, UDP_PORT), bufferSize=BUFFER_SIZE):
        self.sender_addr = 0
        self.seq_num = 0
    ...
    def send(self, data):
        if self.isServer:
            ...
            self.seq_num = sock_send(
                data, self.UDPSocket, self.seq_num, self.sender_addr)
        else:
            """ print("Sending to server") """
            # self.UDPSocket.sendto(data, self.addressPort)
            self.seq_num = sock_send(
                data, self.UDPSocket, self.seq_num, self.addressPort)
    ...
    def receive(self):
        ...
        data, self.seq_num, self.sender_addr = sock_receive(
            self.UDPSocket, self.seq_num)
